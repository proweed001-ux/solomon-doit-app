...
export default function BillPage() {
  const { selectedPS, selectedDate, storeCols, todData, exportBillCSV, setScreen, showToast } = useAppStore();
  ...
  const billData = useMemo(() => {
    const map: Record<string, { sku: string; qty: number; skuCode: string; defaultPrice: number }[]> = {};
    for (const store of storeCols) {
      map[store] = [];
      for (const row of Object.values(todData)) {
        const q = safeNonNegInt(row.stores?.[store]);
        if (q > 0) {
          // CRITICAL: Use detailAmt (row.amt) only, NEVER fallback to totalInvoiceAmt / totalPcs
          // detailAmt is the sum of line-level amounts from raw data
          // If detailAmt is null (missing row.amt), use 0 as default price
          const defPrice = row.detailAmt != null && row.detailAmt > 0 ? row.detailAmt : 0;
          map[store].push({ sku: row.sku, qty: q, skuCode: row.skuCode, defaultPrice: defPrice });
        }
      }
  ...
  const storeSums = useMemo(() => {
    ...
    for (const [st, items] of Object.entries(billData)) {
      sums[st] = (items as any[]).reduce((s: number, i: any) => s + i.qty, 0);
    }
    return sums;
  }, [billData]);

  const grandTotal = Object.values(storeSums).reduce((a: number, b: number) => a + b, 0);
  
  // Totals for active store
  const activeStoreTotalQty = activeItems.reduce((s: number, i: any) => s + i.qty, 0);
  const activeStoreTotalAmt = activeItems.reduce((s: number, i: any) => {
  ...
  const getCurrentTab = () => {
    ...
    if (activeItems.length) {
      ...
      activeItems.forEach((item, i) => {
        ...
        lines.push(`${i + 1}. ${item.sku} | ${item.qty} ชิ้น x ฿${fmtMoney(p)} = ฿${fmtMoney(p * item.qty)}`);
      });
      lines.push('');
  ...
  const handlePrint = () => {
    if (window.self !== window.top) {
      ...
      // Wait a moment then try anyway, some browsers might allow it
  ...
  const saveEditPrice = (sku: string) => {
    ...
    if (!isNaN(p) && p >= 0) {
      setCustomPrices(prev => ({ ...prev, [sku]: p }));
    }
    setEditingSku(null);
  };
    ...
    <div className="flex flex-col h-full bg-slate-900 print:bg-white print:text-black print:h-auto print:min-h-screen print:block">
      ...
      <header className="sticky top-0 z-50 h-14 bg-slate-900/80 backdrop-blur-xl border-b border-slate-700/50 flex items-center gap-3 px-4 shrink-0 print:hidden">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-bold text-slate-100">สรุปบิล</div>
          <div className="text-xs text-slate-400">{storeCols.length} ร้าน · {Number(grandTotal).toLocaleString('th-TH')} ชิ้น</div>
        </div>
          ...
          <>
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-blue-500/10 to-emerald-500/10 border border-blue-500/20 print:hidden">
              ...
              <div className="text-xs text-slate-500 mt-1">
                วันที่ {displayDateKey(selectedDate || '')} · พนักงานขาย {selectedPS}
              </div>
            </div>
                  ...
                  {activeItems.map((item, i) => {
                    ...
                    return (
                      <tr key={i} className="border-b border-gray-300 print:text-sm">
                        ...
                        <td className="py-2 text-right font-bold">฿{fmtMoney(p * item.qty)}</td>
                      </tr>
                    )
                  })}
                </tbody>
                  ...
                  <div key={i} className="flex flex-col p-3 rounded-xl bg-slate-800/40 border border-slate-700/40 gap-2">
                    ...
                    <div className="flex items-center justify-between border-t border-slate-700/50 pt-2 mt-1">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-[13px] font-extrabold text-white">฿{fmtMoney(currentPrice * item.qty)}</span>
                      </div>
                    </div>
                  </div>
                );
